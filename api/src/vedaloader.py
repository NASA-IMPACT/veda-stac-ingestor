    ...
    def update_collection_summaries(
        ...
        with conn.cursor() as cur:
            with conn.transaction():
                logger.info("Updating dashboard summaries for collection: {}.".format(collection_id))
                cur.execute(
                ...
                cur.execute(
                    "SELECT pgstac.collection_bbox(%s)",
                    collection_id
                )
                logger.info("Updating temporal extent for collection: {}.".format(collection_id))
                cur.execute(
                    "SELECT pgstac.collection_temporal_extent(%s)",
                    collection_id
                )
